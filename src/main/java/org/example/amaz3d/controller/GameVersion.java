...
public abstract class GameVersion implements Serializable {
    ...
    public GameVersion(int length, int width, int nbFloors,int nbObstacles,int nbMonstres,int nbTeleport,int nbDoors,int nbBonus,int typeBonus, String name, Scores score) throws FormatNotSupported, FormatNotSupported {
                    ...
                    if (!isInBounds(goal, current))player.setPosition(start, angle);
                        ...
                        if (wallB != null) slide(start, goal, wallB, radius, angle);
                            ...
                            switch (current.getCase(y, x)) {
                                ...
                                case Maze.BONUS:
                                    pickBonus(x, y);
                                     System.out.println("model.Bonus :"+player.getBonus().size());
                                    break;
                                case Maze.KEY:
                                    pickKey(x, y);
                                    break;
