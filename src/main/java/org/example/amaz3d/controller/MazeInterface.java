...
import org.example.amaz3d.Menu;
...
public interface MazeInterface {
    ...
    static void initSounds() throws URISyntaxException {
        File folder = new File(Constants.getSoundResource("").toURI());
        System.out.println(folder +" "+folder.isDirectory());
        //System.out.println(Constants.getImgResourcePath("") +" "+new File(Constants.getImgResourcePath("")).isDirectory());
    ...
    static void configLabel(Label txt, String path, String style) {
        Image img = new Image(Constants.getImgResourcePath(path));
    ...
    static GameVersion load(String file) throws IOException, ClassNotFoundException {
        FileInputStream fis = new FileInputStream(Constants.getSavingResourcePath(file));
    ...
    static Maze getMaze(int L, int l ) throws FormatNotSupported, URISyntaxException, IOException{
        initSounds();
        if(l==-1){
